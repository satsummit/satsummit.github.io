...
import { generateEventsMDXIndex } from './plugins/events-mdx-index';
...
type EventPeopleTypes = keyof Queries.EventPeople;

type EventPeopleFieldSource = Record<EventPeopleTypes, string[] | undefined>;
...
export const createPages: GatsbyNode['createPages'] = async ({
  ...
  const { data } = await graphql<{ allLetter: { nodes: Queries.Letter[] } }>(`
    query {
 ...
 */
const searchEventForRole = async (
  role: EventPeopleTypes,
  name: string,
  context: any
) => {
  ...
  const events: Queries.Event[] = Array.from(entries);
  ...
  const filtered = events.filter((event) =>
    ...
    (event.people as unknown as EventPeopleFieldSource)?.[role]?.includes(name)
  );

  return filtered.map(
    (event) => ({ role, event }) as { role: string; event: Queries.Event }
  );
};
 ...
 */
const eventPeopleResolver = (role: EventPeopleTypes) => {
  return {
    type: '[PeopleOrVoid]',
    resolve: async (
      source: EventPeopleFieldSource,
      args: any,
      context: any
    ): Promise<Queries.PeopleOrVoid[]> => {
      ...
      const queryResult: Queries.People[] = Array.from(entries);
